...
describe('connectDatadog', () => {
  ...
  describe('when options are passed to the closure', () => {
    ...
    describe('routeNameMapper', () => {
      describe('when routeNameMapper option is provided', () => {
        it('should append route_name tag based on the based on the mapper result', async () => {
          const route_name_mapper = path => `route-${path}`
          const stat = 'node.express.router'
          const statTags = [
            `route:${req.route.path}`,
            `response_code:${res.statusCode}`,
            `route_name:${route_name_mapper(req.path)}`,
          ]

          await asyncConnectDatadogAndCallMiddleware({ response_code: true, route_name_mapper })
          expectConnectedToDatadog(stat, statTags, false)
        })
      })
    })
